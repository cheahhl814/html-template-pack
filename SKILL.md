...
**Pattern**: left-side sticky sidebar with 5 icon+label tabs (Summary / Metrics / Methodology / Findings / Annotations), responsive collapse to horizontal strip on screens <720px, print shows all panels sequentially. Each panel is a `<section data-panel="..." id="panel-...">` inside `.report-panels`. Tab switching is driven by `__reportGoTo(id)` exposed on `window` for the v2 annotation engine's `jumpTo` (drawer click → activate panel → scroll to mark). Keyboard navigation: Up/Down/Arrow keys in sidebar, Home/End for first/last.
